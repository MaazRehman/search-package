...
jest.mock('../SearchPackage/SearchPackage', () => () => <div data-testid="search-package">Mocked SearchPackage Component</div>);
jest.mock('../Loader/Loader', () => ({ tip }: { tip: string }) => <div data-testid="loader">{tip}</div>);
...
describe('Packages', () => {
    ...
    test('renders table when loading is false', () => {
        ...
        (usePackageInfo as jest.Mock).mockReturnValue({
            packages: [
                { name: 'package1', owners: 'https://example.com/package1', stars: 100 },
                { name: 'package2', owners: 'https://example.com/package2', stars: 200 },
            ]
        });
